...
class Code(commands.Cog):
        ...
        while i < len(lines):
            ...
            if parameter_name not in [
                ...
                if parameter_name in parameters:
                    await ctx.send("The parameter `" + parameter_name +
                                           "` is already provided.")
                    return
                parameters[parameter_name] = parameter_value
            i += 1
        if "code" not in parameters:
            await ctx.send("Please provide the code!")
            return
        if not code_language:
    ...
    async def reset(self, ctx):
        ...
        if ctx.message.author.id in self.users_configuration:
            ...
            await ctx.send("You hadn't any configured settings.")
    ...
    async def engine(self,
        ...
        if not language_name:
            ...
            return
        if not engine_name:
            await ctx.send("Please specify an engine.")
            return
        found = False
    ...
    async def show(self, ctx):
        ...
        if ctx.message.author.id not in self.users_configuration:
            await ctx.send("You don't have any settings set.")
            return
        msg = "```Markdown\nSettings\n==========\n\n"
